...
def joern_parse(joern_path, input_path, output_path, file_name):
    ...
    # print("\nOutput: " + str(joern_parse_call))
    return out_file


def joern_export(joern_path, path_to_cpg_file, output_dir):
    repr = "all"  # all|ast|cdg|cfg|cpg|cpg14|ddg|pdg - defaults to `Cpg14`
    format = "graphson"  # dot|graphml|graphson|neo4jcsv - defaults to `Dot`

    joern_parse_call = subprocess.run([
        joern_path + "joern-export", path_to_cpg_file, "-o", output_dir, "--repr", repr, "--format", format
    ], stdout=subprocess.PIPE)
...
def joern_create(joern_path, in_path, out_path, cpg_files):
    joern_process = subprocess.Popen([joern_path + "joern"], stdin=subprocess.PIPE, stdout=subprocess.PIPE, shell=True)
    json_files = []
    for cpg_file in cpg_files:
        json_file_name = f"{cpg_file.split('.')[0]}.json"
        json_files.append(json_file_name)

        print(f"Json-ising {in_path+cpg_file}")
        
        if os.path.exists(in_path+cpg_file):
            json_out = f"{os.path.abspath(out_path)}/{json_file_name}"
            
            # Run this cmd to import the cpg file
            ...
            print(f"[Warning] File {cpg_file} not found.")
