...
from utils.model import load_model_tokenizer, add_special_token
from utils.config import get_config, setup_cuda, set_seed

from utils.model import MultiPathCodeBERT as ModelOfInterest
from utils.data import DefectDataset4Paths as CustomDataset
...
def evaluate(logger, args, model, tokenizer, file_path, path_file, eval_when_training=False):
	eval_output_dir = args.output_dir
	
	_dataset = CustomDataset(tokenizer, args, file_path=file_path, path_file=path_file)
	...
	def eval_fn():
		...
		start_idx = 0
		result_dict = dict()
		for batch in tqdm(eval_dataloader, desc="Evaluating", position=0, leave=True, total=len(eval_dataloader)):
			...
			with torch.no_grad():
				...
				labels.extend(batch[2].detach().cpu().numpy().tolist())
				# If prob > 0.5, then pred = 1 else pred = 0
				pred = (prob >= 0.5).int()
				preds.extend(pred.detach().cpu().numpy().tolist())
				
			# # Save the predictions and labels, where key is the index of the sample in the dataset
			f_idxs = _dataset.get_idxs_for_a_batch(start_idx=start_idx, end_idx=start_idx + batch[0].shape[0])
			for idx, f_idx in enumerate(f_idxs):
				# Sanity check: label here should be the same as the label in the dataset
				assert int(batch[2][idx]) == _dataset.examples[start_idx + idx].label
				result_dict[f_idx] = {
					"pred": int(pred[idx]),
					"label": int(batch[2][idx])
				}
		...
		with open(os.path.join(eval_output_dir, 'results.json'), 'w') as f:
		...
		# Precision
		eval_precision = np.mean(labels[labels == 1] == preds[labels == 1])
		# Recall
		eval_recall = np.mean(labels[preds == 1] == preds[preds == 1])
		# F1
...
def train(args, logger):
	if args.wandb_logging:
		wandb.init(project="VulDetect", name="multiPathCodebert-devign")
	...
	train_dataset = CustomDataset(tokenizer, args, file_path=args.train_data_file, path_file=args.train_path_file)
	...
	eval_fn = evaluate(logger, args, model, tokenizer, file_path=args.eval_data_file, path_file=args.eval_path_file,
	...
	for idx in range(args.num_train_epochs):
		tr_num = 0
		for step, batch in tqdm(enumerate(train_dataloader), desc=f"Epoch {idx}", position=0, leave=True,
